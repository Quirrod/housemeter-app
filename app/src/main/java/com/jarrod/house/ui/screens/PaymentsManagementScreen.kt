        ...
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            if (isLoading) {
                ...
                val filteredPayments = when (selectedTab) {
                    ...
                    1 -> payments.filter { it.status == "approved" }
                    2 -> payments.filter { it.status == "rejected" }
                    else -> payments
                }
                ...
                if (filteredPayments.isEmpty()) {
                    Box(
                        ...
                        Column(
                            ...
                            Icon(
                                when (selectedTab) {
                                    0 -> Icons.Default.PendingActions
                            ...
                            Text(
                                text = when (selectedTab) {
                                    ...
                                    1 -> "No hay pagos aprobados"
                                    2 -> "No hay pagos rechazados"
                    ...
                    LazyColumn(
                        ...
                        items(filteredPayments.sortedByDescending { it.payment_date }) { payment ->
                            PaymentManagementCard(
                                payment = payment,
                                onApprove = if (payment.status == "pending") {
                                    { paymentViewModel.updatePaymentStatus(context, payment.id, "approved", null) }
                                } else null,
                                onReject = if (payment.status == "pending") {
                                    { paymentViewModel.updatePaymentStatus(context, payment.id, "rejected", null) }
                                } else null,
                                onDelete = {
        ...
        Column(
            ...
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    // Payment details
                    payment.debt_description?.let { desc ->
                        Text(
                            text = desc,
